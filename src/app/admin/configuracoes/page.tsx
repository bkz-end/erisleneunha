      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-semibold text-gray-800">Configuracoes</h1>
          </div>
        </div>
      </header>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="p-6">
                ...
                <div className="flex items-center gap-2 mt-1">
                  {subscription?.status === "active" ? (
                    ...
                    <>
                      ...
                      <span className="font-semibold text-yellow-600">Periodo de Teste</span>
                    </>
                  ) : (
                    <>
            ...
            {subscription?.status === "trial" && daysRemaining <= 3 && (
              <div className={`p-4 rounded-xl mb-6 ${isTrialExpired ? "bg-red-50 border border-red-200" : "bg-yellow-50 border border-yellow-200"}`}>
                <p className={`text-sm ${isTrialExpired ? "text-red-700" : "text-yellow-700"}`}>
                  {isTrialExpired 
                    ? "Seu periodo de teste expirou! Assine agora para continuar usando."
                    : `Seu periodo de teste termina em ${daysRemaining} dia${daysRemaining > 1 ? "s" : ""}. Assine agora para nao perder acesso!`
                  }
                </p>
              </div>
            )}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <ul className="text-sm space-y-1 text-rose-gold-light">
                    ...
                    <li>✓ Gestao de servicos</li>
                    <li>✓ Notificacoes WhatsApp</li>
            ...
            {subscription?.status === "active" && (
              <div className="text-center py-4 bg-green-50 rounded-xl">
                <p className="text-green-600 font-medium">Sua assinatura esta ativa!</p>
                {subscription.expires_at && (
                  <p className="text-sm text-green-500 mt-1">
                    Proxima cobranca: {new Date(subscription.expires_at).toLocaleDateString("pt-BR")}
                  </p>
                )}
              </div>
            )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Numero para onde os clientes serao redirecionados apos agendar
            </p>
          </div>
