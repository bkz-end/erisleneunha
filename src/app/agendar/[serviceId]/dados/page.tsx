...
export default function DadosClientePage({ params }: PageProps) {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        const targetWhatsApp =
          adminWhatsApp || process.env.NEXT_PUBLIC_ERISLENE_WHATSAPP || "5511999999999";
  ...
  if (success) {
    ...
    const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(
      ...
      `Agendamento de ${service?.name}\nValor: ${formatPrice(service?.price || 0)}`
    )}&location=${encodeURIComponent("Salão de Beleza")}`;

    return (
      <main className="min-h-screen bg-gradient-luxury relative overflow-hidden flex items-center justify-center p-4">
        <div className="absolute top-0 left-0 w-96 h-96 bg-pastel-rose/30 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2" />
        <div className="absolute bottom-0 right-0 w-96 h-96 bg-pastel-peach/30 rounded-full blur-3xl translate-x-1/2 translate-y-1/2" />
        <div className="relative z-10 text-center w-full max-w-md">
          <div className="card-glass p-6 sm:p-10">
            <div className="w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-6 rounded-full bg-green-100 flex items-center justify-center animate-bounce">
              <svg className="w-8 h-8 sm:w-12 sm:h-12 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h2 className="font-display text-xl sm:text-3xl text-rose-gold-dark mb-2">
              Agendamento solicitado!
            </h2>
            <p className="text-stone-500 mb-6">Agora é só confirmar no WhatsApp.</p>

            <div className="bg-pastel-cream rounded-xl p-4 mb-6 text-left">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-stone-500">Serviço:</span>
                  <span className="font-medium text-stone-700">{service?.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-stone-500">Data:</span>
                  <span className="font-medium text-stone-700">
                    {new Date(bookingDateTime).toLocaleDateString("pt-BR", {
                      weekday: "short",
                      day: "2-digit",
                      month: "short",
                    })}
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-rose-gold">{bookingTime}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-stone-500">Valor:</span>
                  <span className="font-semibold text-rose-gold-dark">{formatPrice(service?.price || 0)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <a
                ...
                className="flex items-center justify-center gap-2 w-full py-3 px-4 bg-white border-2 border-blue-500 text-blue-600 rounded-xl font-medium hover:bg-blue-50 transition-colors"
              >
              ...
              <p className="text-stone-400 text-sm">Redirecionando para WhatsApp em instantes...</p>
          ...
          <Link href="/agendar" className="inline-block mt-6 text-rose-gold hover:text-rose-gold-dark text-sm">
  ...
  return (
    <main className="min-h-screen bg-gradient-luxury relative overflow-hidden">
      ...
      <div className="relative z-10">
        <header className="pt-10 sm:pt-16 pb-6 sm:pb-8 px-4 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/60 backdrop-blur-sm rounded-full mb-6 border border-rose-gold-light/30">
            ...
            <span className="text-sm text-rose-gold-dark font-medium">Último passo</span>
          </div>

          <h1 className="font-display text-3xl sm:text-4xl md:text-5xl text-rose-gold-dark mb-4">
            Seus <span className="text-gold-gradient">dados</span>
          </h1>

          <p className="text-stone-500 text-base sm:text-lg">Preencha para confirmar o agendamento</p>
        ...
        <div className="flex justify-center items-center gap-2 sm:gap-3 mb-8 sm:mb-10 px-4">
          <div className="flex items-center gap-2 sm:gap-3 bg-white/60 backdrop-blur-sm px-4 sm:px-6 py-2.5 sm:py-3 rounded-full border border-rose-gold-light/30">
            <div className="flex items-center gap-2">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-rose-gold-light text-rose-gold-dark flex items-center justify-center text-xs sm:text-sm font-semibold">
                1
              </span>
              <span className="text-rose-gold hidden sm:inline">Serviço</span>
            </div>
            <div className="w-8 sm:w-12 h-px bg-rose-gold" />
            <div className="flex items-center gap-2">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-rose-gold-light text-rose-gold-dark flex items-center justify-center text-xs sm:text-sm font-semibold">
                2
              </span>
              <span className="text-rose-gold hidden sm:inline">Horário</span>
            </div>
            <div className="w-8 sm:w-12 h-px bg-rose-gold" />
            <div className="flex items-center gap-2">
              ...
              <span className="text-rose-gold-dark font-medium hidden sm:inline">Dados</span>
            </div>
          </div>
        </div>

        <div className="max-w-lg mx-auto px-4 pb-20">
          <div className="card-luxury p-5 sm:p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} className="card-luxury p-5 sm:p-6">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className={`input-elegant ${errors.name ? "border-red-300 focus:border-red-400" : ""}`}
                />
                {errors.name && (
              ...
              <div>
                ...
                <input
                  ...
                  className={`input-elegant ${errors.whatsapp ? "border-red-300 focus:border-red-400" : ""}`}
                />
                {errors.whatsapp && (
                  <p className="mt-2 text-sm text-red-500 flex items-center gap-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="mt-6 sm:mt-8 flex gap-3">
              ...
              <button
                ...
                className={`flex-1 py-3 px-4 rounded-xl text-center font-medium transition-all duration-300 ${
                  submitting ? "bg-neutral-elegant text-stone-400 cursor-not-allowed" : "btn-luxury"
                }`}
              >
                {submitting ? (
        ...
        <div className="text-center pb-8">
          <p className="text-stone-400 text-sm">Unhas novas, vida nova.</p>
        </div>
      </div>
    </main>
  );
